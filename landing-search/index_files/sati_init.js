...
if (!window.admp_) {
    ...
    var adtlgcen_SETTINGS = {
		...
		SURVEY_URL: "http://www.surveygizmo.com/s3/3820231/SATI-",
    ...
    adtlgcen.util.send = function (url, skipExtraparams, pixelImage) {
    	if(!skipExtraparams){
    		if(window[enr_vars.evid] != "-entered"){
    ...
    adtlgcen.util.survey.onStart = function(hasLogo){
    	var survey_url = adtlgcen_SETTINGS.SURVEY_URL + "?evid="+window[enr_vars.evid];
    ...
    adtlgcen.util.survey.show = function(){
	    ...
	    var sg_div = document.createElement("div");
	    sg_div.innerHTML =
	    	"<div style='margin:0.5em'>&nbsp;<span style='float:right;cursor:pointer;font-size:2em' onclick='return adtlgcen.util.survey.onCancel("+(logoURL ? "1" : "0")+");'>&times;</span></div>"+
	      "<div>" +
	        "<div style='width:40%;display:inline-block'>" + (logoURL ? "<img style='max-width:60%;max-height:60%;display:block;margin:auto;padding-bottom:1em' src='"+logoURL+"' />" : "") + "</div>" +
	        "<div style='width:50%;display:inline-block'>" +
	          (SATI_logoURL ? "<img style='max-width:55%;max-height:55%;display:block;margin:auto' src='"+SATI_logoURL+"' />" : "") +
	          "<div style='font:bold 1.1em arial, sans-serif;color: #5d6268;text-align:center;margin-top:0.5em'>Studiul de Audienta Si Trafic<br />Internet (www.sati.ro)</div>" +
	        "</div>" +
	      "</div>" +
	      "<div style='background-color:#5d6268;height:0.5em;margin:0.5em 0.5em 1px'></div>" +
	      "<div style='background-color:#943532;height:0.5em;margin:0 0.5em'></div>" +
	      "<div style='margin:2em;letter-spacing:0.1px'>" +
	        "<div>" +
	          "<div style='font:bold 2em arial, sans-serif'>Va rugam sa ne ajutati sa masuram audienta "+window.location.hostname+"</div>" +
	          "<div style='font:bold 1.2em arial, sans-serif;color: #5d6268'>" +
	          	"<div style='margin-top:1em'>Ati fost selectat aleator sa participati la Studiul de Audienta si Trafic Internet, singurul studiu al mediului online din Romania.</div>" +
	          	"<div style='margin-top:1em'>BRAT va invita sa completati chestionarul urmator, dureaza doar cateva minute.</div>" +
	          	"<div style='margin-top:1em'>Va multumim!</div>" +
	          "</div>" +
	        "</div>" +
	        "<div style='text-align:center;margin:1.2em'>" +
	          "<div style='cursor:pointer;font:bold 1.2em arial, sans-serif;letter-spacing:1.5px;color:white;background-color:#943532;padding:0.5em;display:inline-block' onclick='return adtlgcen.util.survey.onStart("+
	          (logoURL ? "1" : "0")+");'>completeaza &gt;</div>" +
	        "</div>" +
	      "<div>";
	    ...
	    sg_div.style.left = "0";
	    sg_div.style.right = "0";
	    sg_div.style.top = "0";
	    sg_div.style.margin = window.matchMedia("(min-width: 75em)").matches ? "10% 34% 0" : "10% auto 0";
	    sg_div.style['min-width'] = "25em";
	    sg_div.style.backgroundColor = "#ffffff";
	    sg_div.style.borderColor = "#000000";
	    sg_div.style.borderStyle = "solid";
	    sg_div.style.zIndex = "50001";
    ...
    adtlgcen.util.processTaskQueue = function(){
    	var task = adtlgcen.taskQueue.pop();
    	while(task){
    		if("optOut"==task){
    		...
    		task = adtlgcen.taskQueue.pop();
    	}
    	window.setTimeout(function(){
    		adtlgcen.util.processTaskQueue();
        }, 1000);
    };
